...
def reemplazar_ubicacion(ubicacion_id: int, nuevo: Ubicacion, session: Session = Depends(get_session)):
    u = session.get(Ubicacion, ubicacion_id); 
    if not u: raise HTTPException(status_code=404, detail="No encontrado")
    for k,v in nuevo.dict().items(): setattr(u, k, v)
    session.add(u); session.commit(); session.refresh(u); return u
...
def actualizar_ubicacion(ubicacion_id: int, data: Ubicacion, session: Session = Depends(get_session)):
    u = session.get(Ubicacion, ubicacion_id)
    if not u: raise HTTPException(status_code=404, detail="No encontrado")
    for key, value in data.dict(exclude_unset=True).items(): setattr(u, key, value)
    session.add(u); session.commit(); session.refresh(u); return u
@router.delete('/{ubicacion_id}')
def eliminar_ubicacion(ubicacion_id: int, session: Session = Depends(get_session)):
    u = session.get(Ubicacion, ubicacion_id)
    if not u: raise HTTPException(status_code=404, detail="No encontrado")
    session.delete(u); session.commit(); return {'ok': True}
