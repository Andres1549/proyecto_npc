...
def obtener_vendedor(vendedor_id: int, session: Session = Depends(get_session)):
    ...
    if not v: raise HTTPException(status_code=404, detail="No encontrado")
    return v
@router.post('/', response_model=Vendedor)
def crear_vendedor(v: Vendedor, session: Session = Depends(get_session)):
    session.add(v); session.commit(); session.refresh(v); return v
@router.put('/{vendedor_id}', response_model=Vendedor)
def reemplazar_vendedor(vendedor_id: int, nuevo: Vendedor, session: Session = Depends(get_session)):
    v = session.get(Vendedor, vendedor_id); 
    if not v: raise HTTPException(status_code=404, detail="No encontrado")
    for k, val in nuevo.dict().items(): setattr(v, k, val)
    session.add(v); session.commit(); session.refresh(v); return v
...
def actualizar_vendedor(vendedor_id: int, data: Vendedor, session: Session = Depends(get_session)):
    v = session.get(Vendedor, vendedor_id)
    if not v: raise HTTPException(status_code=404, detail="No encontrado")
    for key, value in data.dict(exclude_unset=True).items(): setattr(v, key, value)
    session.add(v); session.commit(); session.refresh(v); return v
@router.delete('/{vendedor_id}')
def eliminar_vendedor(vendedor_id: int, session: Session = Depends(get_session)):
    v = session.get(Vendedor, vendedor_id)
    if not v: raise HTTPException(status_code=404, detail="No encontrado")
    v.activo = False; session.add(v); session.commit(); return {'ok': True}
@router.post('/{vendedor_id}/items/{item_id}')
def vincular_item(vendedor_id: int, item_id: int, session: Session = Depends(get_session)):
    v = session.get(Vendedor, vendedor_id); item = session.get(Item, item_id)
    if not v or not item: raise HTTPException(status_code=404, detail="No encontrado")
    session.add(VendedorItemLink(vendedor_id=v.id, item_id=item.id)); session.commit(); return {'ok': True}
...
def desvincular_item(vendedor_id: int, item_id: int, session: Session = Depends(get_session)):
    ...
    if not link: raise HTTPException(status_code=404, detail="No encontrado")
    session.delete(link); session.commit(); return {'ok': True}
